...
export function ControlRoomManager() {
  ...
  // Use the new hook to manage Control playback and WebSocket sync
  useControlPlayback(connectionStatus === 'connected' ? roomId || undefined : undefined);
  ...
  // 2. Synchronize queue state and events when connected
  useEffect(() => {
  ...
  if (connectionStatus === 'disconnected' || (connectionStatus === 'error' && !roomId)) {
    return (
      <div className="fixed inset-0 z-50 bg-space-950/95 backdrop-blur-xl flex items-center justify-center p-4">
        <div className="max-w-md w-full p-8 rounded-2xl bg-space-900 border border-cyan-500/30 shadow-[0_0_50px_rgba(24,216,255,0.15)] text-text-primary">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 bg-cyan-500/10 rounded-xl border border-cyan-500/30 text-cyan-500">
              <KeyRound className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-xl font-display uppercase tracking-widest text-text-primary">
                Link Control Unit
              </h2>
          ...
          <form onSubmit={handleManualJoin} className="space-y-4">
            <div>
              <input
                ...
                autoFocus
                className="w-full bg-cyan-500/5 border border-cyan-500/30 rounded-xl py-4 px-4 text-center font-mono text-2xl tracking-[0.3em] uppercase placeholder-text-secondary/40 focus:outline-none focus:shadow-[0_0_20px_rgba(24,216,255,0.3)] transition-all"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-4 rounded-xl bg-cyan-500 text-space-950 font-display uppercase tracking-widest font-bold flex items-center justify-center gap-2 hover:bg-cyan-500/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-glow-active"
            >
              {submitting ? (
